...
**Deliverables:**
- `BlueBubbles.Windows` — WinUI3 Packaged App (.NET 8, Windows 10 19041+)
...
- `MainWindow.xaml.cs` (`HideToTray`), `App.xaml.cs` (DI + `ReconcileStartupState`), `Package.appxmanifest` (startupTask extension)
...
**Decision:** ship as a **free, unpackaged, self-contained `.exe` installer** (no MSIX, no certificate) for the public GitHub release. The build bundles the .NET + Windows App SDK runtimes, so it runs on a clean machine with no prerequisites. Trade-off: the unsigned `.exe` shows a one-time SmartScreen "unknown publisher" prompt until reputation builds (or until code signing is added). A signed-MSIX path (`package.ps1`) is kept as an alternative for later.
...
- **`package.ps1`** + **`signing/`** — alternative signed-MSIX sideload path (self-signed test cert).
